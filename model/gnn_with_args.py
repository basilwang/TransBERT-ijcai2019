...
class Bert_EventGraph_With_Args(Module):
    def __init__(self, bert_config, tokenizer, id_word, vocab_size, hidden_dim, word_vec, L2_penalty, MARGIN, LR, T,
                 BATCH_SIZE=1000, dropout_p=0.2):
        ...
        self.optimizer = optim.RMSprop(
            [{'params': tune_params}, {'params': self.embedding.parameters(), 'lr': LR * 0.06}], lr=LR,
    ...
    def compute_scores(self, hidden, metric='euclid'):  # batch_size*13*128
        ...
        if metric == 'dot':
            ...
            scores = self.multi[0] * self.metric_euclid(a, b) + self.multi[1] * self.metric_dot(a, b) + self.multi[
                2] * self.metric_cosine(a, b)
        return scores
    ...
    def correct_answer_position(self, L, correct_answers):
        ...
        num_correct1.data[0], len(correct_answers), num_correct1 / len(correct_answers) * 100.))
        print("%d / %d 2ed max correct: %f" % (
        num_correct2.data[0], len(correct_answers), num_correct2 / len(correct_answers) * 100.))
        print("%d / %d 3rd max correct: %f" % (
        num_correct3.data[0], len(correct_answers), num_correct3 / len(correct_answers) * 100.))
        print("%d / %d 4th max correct: %f" % (
        num_correct4.data[0], len(correct_answers), num_correct4 / len(correct_answers) * 100.))
        print("%d / %d 5th max correct: %f" % (
        num_correct5.data[0], len(correct_answers), num_correct5 / len(correct_answers) * 100.))
    ...
    def predict_with_minibatch(self, input, A, targets, dev_index, metric='euclid'):
        ...
        for i in range(int(len(targets) / self.batch_size)):
            scores[i * self.batch_size:(i + 1) * self.batch_size] = self.forward(
                input[i * self.batch_size:(i + 1) * self.batch_size], A[i * self.batch_size:(i + 1) * self.batch_size],
                metric)
