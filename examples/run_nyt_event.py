...
def read_nyt_event_examples(input_file, is_training):
    examples = []
...
def convert_examples_to_features(examples, tokenizer, max_seq_length,
    ...
    for example_index, example in enumerate(examples):
        context_tokens = tokenizer.tokenize(example.ei)

        choices_features = []
        for ending_index, ending in enumerate(example.next):
...
def main(MARGIN=0.15):
    ...
    if args.do_test and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        test_examples = read_nyt_event_examples(os.path.join(args.data_dir, 'testnew.csv'), is_training = False)
